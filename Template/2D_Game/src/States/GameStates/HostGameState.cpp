...
HostGameState::HostGameState(Application *a_application) : IGameState(a_application)
{
	m_blockUpdate = true;
	m_blockDraw = true;
	Load();
...
void HostGameState::Update(float a_dt)
{
	for (int i = 0; i < m_allButtons.size(); i++)
		m_allButtons[i]->Update();
}

void HostGameState::Draw(SpriteBatch_Imidiate *a_SBI)
{
	m_SBI = a_SBI;
...
void HostGameState::DrawBackground()
{
